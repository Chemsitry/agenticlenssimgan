"""
simulate_v7.py — v6 pipeline + VELA cosmological-sim source galaxies

Base: simulate_v6.py (INTERPOL real-galaxy pipeline)

v7 change (only one):
  - SOURCE light: switch from real COSMOS-Web cutouts (z~0.5, observed,
    PSF-convolved) to VELA cosmological hydrodynamic simulation galaxies
    (intrinsic emission at correct source redshift, post-processed with
    sunrise dust radiative transfer).
  - LENS light: unchanged — still real COSMOS-Web elliptical stamps.

Source stamps now load from prepped_mosaic_630/vela_sources/ instead of
prepped_mosaic_630/sources/. Format and INTERPOL handling are identical.

Requires:
    prep_vela_v7.py    populates prepped_mosaic_630/vela_sources/
    prep_stamps_v6.py  populates prepped_mosaic_630/lenses/      (lens light)

Usage:
    .venv/bin/python3 prep_vela_v7.py             # extract VELA source stamps
    .venv/bin/python3 simulate_v7.py              # 10 test images
    .venv/bin/python3 simulate_v7.py --n 2000     # full dataset
    .venv/bin/python3 simulate_v7.py --sersic     # fallback to v4-style Sersic
"""
...
if USE_SERSIC:
    ...
    # Lens stamps — v7 uses larger 201x201 stamps with narrower taper to fix
    # the visible "force field" boundary that v6's 101x101 stamps produced.
    lens_dir = PREPPED_DIR / 'lenses_v7'
    if lens_dir.exists():
        try:
            lens_stamps = {}
            for band in BANDS:
...
def simulate_one(lensed=True, seed=None):
    ...
    for band in BANDS:
        ...
        # PSF setup (v7 split): source uses real PSF (because VELA pristine
        # is unconvolved — we want PSF after lensing), lens light uses delta
        # PSF (because COSMOS-Web stamps are already PSF-convolved). For the
        # Sersic fallback both use the real PSF.
        if use_interpol_src and use_interpol_lens:
            psf_for_src = make_psf_obj(band)
            psf_for_lens = make_delta_psf()
            kwargs_num = KWARGS_NUMERICS_INTERPOL
        else:
            psf_for_src = make_psf_obj(band)
            psf_for_lens = make_psf_obj(band)
        ...
        # Two ImageModels: one for source (real PSF), one for lens light
        # (delta PSF). Same data_class, lens model, and light models — only
        # the PSF differs.
        im_src = ImageModel(
        ...
        im_lens = ImageModel(
            data_class=data_class, psf_class=psf_for_lens,
            lens_model_class=lens_model,
            source_model_class=source_model,
            lens_light_model_class=lens_light_model,
            kwargs_numerics=kwargs_num)

        # Calibrate lens amp using lens-PSF model
        kw_ll_cal = [{**kwargs_lens_light[0], 'amp': 1.0}]
        kw_src_cal = [{**kwargs_source[0], 'amp': 1.0}]

        img_lens_unit = im_lens.image(kwargs_lens, kw_src_cal,
        ...
        # Calibrate source amp using src-PSF model
        ...
        # Final render: lens with delta PSF, source with real PSF, then sum
        kw_ll = [{**kwargs_lens_light[0], 'amp': amp_lens}]
        kw_src = [{**kwargs_source[0], 'amp': amp_src}]

        image_lens_part = im_lens.image(kwargs_lens, kw_src,
                                          kwargs_lens_light=kw_ll,
                                          source_add=False, lens_light_add=True)
        image_src_part = im_src.image(kwargs_lens, kw_src,
                                        kwargs_lens_light=kw_ll,
                                        source_add=True, lens_light_add=False)
        image = image_lens_part + image_src_part
        image_source = image_src_part  # source-only output (no lens light)
...
def _simulate_worker(args):
    ...
    for band in BANDS:
        ...
        out['galaxies'][band] = img_noisy.astype(np.float32)
...
def make_preview(all_images, all_sources, labels, theta_Es, z_lenses, z_sources, N):
    ...
    plt.suptitle(f'simulate_v7 [{mode}] — {N} images (630px, 18.9" FoV)', fontsize=12)
    plt.tight_layout()
...
def main():
    global OUT_DIR
    parser = argparse.ArgumentParser(description='v7 multi-band lens simulation (VELA INTERPOL sources)')
    ...
    parser.add_argument('--out_dir', default=None, help='Output directory (default: output/v6)')
    ...
    print(f"\nsimulate_v7 — VELA INTERPOL source pipeline")
